# AI Assistant Contract — aiproviders
...
2. **Run AI tasks** across providers:
   ...
   - Planner: chooses intent and output format from instruction, conversation, context
   - Classify: intent classification with labels
   - Generate: document generation with instruction, conversation, context

3. **Structured outputs**:
   - JSON schemas for planner, classify, generate (in `schemas/*.json`)
   - Action templates in `actions/*.json`, format configs in `formats/*.json`

### Supported Providers

OpenAI, Anthropic, Google, Mistral, Meta, Scaleway
...
- `src/aiproviders/providers/`: AI provider implementations
- `src/aiproviders/providers/actions/`: Action templates with placeholders
- `src/aiproviders/providers/formats/`: Output type configs
- `src/aiproviders/providers/schemas/`: JSON schemas
- `src/aiproviders/commands/`: Command infrastructure for CLI system
...
Providers must inherit from `AIProviderBase`:

```python
from aiproviders.providers import AIProviderBase
...
- `{provider}_planner(**kwargs)`: Choose intent + format from instruction, conversation, context
- `{provider}_classify(**kwargs)`: Intent classification
- `{provider}_generate(**kwargs)`: Document generation
- `_run_llm_provider_impl(payload)`: Low-level LLM dispatch

### Formats and Schemas

- Use `get_action(action_name)`, `get_action_template(action, output_type)`, `get_output_type(format)`, `get_schema(name)` from `AIProviderBase`
- Define action templates in `actions/*.json` with placeholders (`{{instruction}}`, `{{context}}`, `{{conversation}}`, etc.)
- Define format configs in `formats/*.json` (description, instruction for system prompt)
- Define JSON schemas in `schemas/*.json` for structured output
...
- Provider-specific variables:
  - Use provider-specific prefixes (e.g. `OPENAI_`, `SCALEWAY_`, `ANTHROPIC_`, etc.)
...
aiproviders discovers commands from:
...
- [ ] Providers inherit from AIProviderBase correctly
- [ ] Providers implement required services (planner, classify, generate, _run_llm_provider_impl)
- [ ] Formats and schemas are used for prompts and structured output
